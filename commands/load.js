...
module.exports = {
    ...
    backup.fetch(backupID).then(async () => {
      ...
      .setAuthor(message.author.username, message.author.displayAvatarURL())
      .setDescription(`:warning: | When the backup is loaded, all the channels, roles, etc. will be replaced! Type **${mainprefix}confirm** to confirm!!`)
      .setFooter(message.guild.name, guildicon2)

         message.channel.send(warning);
            await message.channel.awaitMessages(m => (m.author.id === message.author.id) && (m.content === mainprefix + "confirm"), {
                max: 1,
                time: 20000,
                errors: ["time"]
            }).catch((err) => {
